...
def ploy_fig(ticker, df, skip_macd_sell="Yes"):
    ...
    stoch = ta.momentum.StochasticOscillator(df["High"], df["Low"], df["Close"], window=80, smooth_window=5)
    df["K"] = stoch.stoch()
    ...
    df["MACD_buy_signal"] = np.where((df["MACD"] > df["MACD_signal"]) & (df["MACD"].shift(1) <= df["MACD_signal"].shift(1)),
                                     df["Close"], np.nan)
    df["MACD_sell_signal"] = np.where(
        (df["MACD"] < df["MACD_signal"]) & (df["MACD"].shift(1) >= df["MACD_signal"].shift(1)), df["Close"], np.nan)

    df["OBV"] = ta.volume.OnBalanceVolumeIndicator(df["Close"], df["Volume"]).on_balance_volume()

    # Calculate CMF (20-day is standard)
    df["CMF"] = ta.volume.ChaikinMoneyFlowIndicator(
        high=df["High"], low=df["Low"], close=df["Close"], volume=df["Volume"], window=20
    ).chaikin_money_flow()

    macd1 = ta.trend.MACD(df["Close"], window_slow=34, window_fast=5, window_sign=5)
    df["MACD1"] = macd1.macd()
    ...
    # Create subplots (Expanded to 6 rows)
    fig = make_subplots(
        rows=5, cols=1, shared_xaxes=True,
        vertical_spacing=0.01,
        row_heights=[4, 2, 2, 1, 1],  # Added row height for CMF
        subplot_titles=[
            f' {ticker}', "Acute MACD", "Volume", "RSI", "Chaikin Money Flow"
        ]
    )

    # Candlestick
    fig.add_trace(
        go.Candlestick(x=df.index, open=df["Open"], high=df["High"], low=df["Low"], close=df["Close"], name="Price"), row=1,
        col=1)
    ...
    # Moved Resistance line outside the loop to prevent duplicate rendering
    resistance = df["Resistance"].iloc[-1]
    fig.add_hline(y=resistance, line_dash="dot", line_color="purple", annotation_text="resistance", row=1, col=1)
    ...
    # Volume
    fig.add_trace(go.Bar(x=df.index, y=df["Volume"], name="Volume", marker_color="green"), row=3, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["VolMA5"], mode="lines", line=dict(color="purple"), name="VolMA5"), row=3,
                  col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["VolMA10"], mode="lines", line=dict(color="pink"), name="VolMA10"), row=3,
                  col=1)

    # RSI
    fig.add_trace(go.Scatter(x=df.index, y=df["RSI_14"], mode="lines", name="RSI"), row=4, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["RSI_5"], mode="lines", name="RSI"), row=4, col=1)
    ...
    # Increased height to 1400 to accommodate the extra row
    fig.update_layout(height=1400, showlegend=True, xaxis_rangeslider_visible=False)
    fig.update_xaxes(
        rangebreaks=[
