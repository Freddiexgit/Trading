...
#         beta = get_stock_market_capitalization(ticker,"beta")
#         stock_beta[ticker] = beta
#     # sorted_dict = dict(sorted(stock_beta.items(), key=lambda item: item[1], reverse=True))
#     with open("resource/us_order_by_beta.csv", 'w') as f:
#         f.write('symbol\n')
#         for key, value in stock_beta.items():
#             f.write(f'{key} , {value}\n')
#     #
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if len(parts) == 2:
                key = parts[0].strip()
                try:
                    value = float(parts[1].strip())  # convert to float
                except ValueError:
                    value = parts[1].strip()  # fallback: keep as string
                result[key] = value
    return result


# Example usage
data = read_dict_from_file("resource/us_order_by_beta.csv")
print(data)
sorted_dict = dict(sorted(data.items(), key=lambda item: item[1], reverse=True))
with open("resource/us_order_by_beta1.csv", 'w') as f:
