...
ft.order(df_tickers, f"resource/{s_str}/us/find_cross_ema_order_by_vol_{s_str}.csv")
df_tickers = pd.read_csv(f"resource/{s_str}/us/find_cross_ema_order_by_vol_{s_str}.csv")
di.generate_pdf(df_tickers, f"resource/{s_str}/us/find_cross_ema_{datetime.now().strftime('%Y-%m-%d-%H-%M')}.pdf", "No", "us")
